...
namespace Utility
{
    ...
    public class CookieHelper
    {
        ...
        /// <returns></returns>
        public static bool Get(string name, string key, string IV, out string value)
        {
            value = string.Empty;
            ...
            try
            {
                string result = CookieHelper.Get(name);
                if (!string.IsNullOrEmpty(result))
                {
                    result = SecurityHelper.DecryptBase64(result);
                    if (!string.IsNullOrEmpty(result))
                    {
                        value = AESHelper.AESDecrypt(result, key, IV);
