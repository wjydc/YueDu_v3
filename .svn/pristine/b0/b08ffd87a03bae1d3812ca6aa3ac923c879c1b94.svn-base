using System;

namespace Model.Common
{
    [Serializable]
    public class PackageOrderInfo : ILogInfo
    {
        #region Fields

        private int _id;
        private int _packageId = 0;
        private string _packageTitle = String.Empty;
        private decimal _fee;
        private DateTime _orderTime;
        private int _status = 0;
        private int _payChannel = 0;
        private string _orderCode = String.Empty;
        private decimal _rebate;
        private decimal _rebateFee;
        private int _feeId = 0;
        private decimal _balance;
        private decimal _bindFee;
        private decimal _cash;
        private int _integral = 0;
        private string _rebateExpression = String.Empty;
        private int _userId = 0;
        private string _userName = String.Empty;
        private int _clientId = 0;
        private string _version = String.Empty;
        private string _userAgent = String.Empty;
        private string _imei = String.Empty;
        private string _imsi = String.Empty;
        private string _channelId = String.Empty;
        private int _sourceType = 0;
        private string _mobile = String.Empty;
        private string _netType = String.Empty;
        private string _province = String.Empty;
        private string _city = String.Empty;
        private string _referUrl = String.Empty;
        private string _iPAddress = String.Empty;
        private string _remoteHost = String.Empty;
        private DateTime _beginTime;
        private DateTime _endTime;
        private DateTime _cancelTime;
        private int _autoRenew = 0;
        private int _orderContentType = 0;

        #endregion

        #region Contructors

        public PackageOrderInfo()
        {

        }

        public PackageOrderInfo
        (
            int id,
            int packageId,
            string packageTitle,
            decimal fee,
            DateTime orderTime,
            int status,
            int payChannel,
            string orderCode,
            decimal rebate,
            decimal rebateFee,
            int feeId,
            decimal balance,
            decimal bindFee,
            decimal cash,
            int integral,
            string rebateExpression,
            int userId,
            string userName,
            int clientId,
            string version,
            string userAgent,
            string imei,
            string imsi,
            string channelId,
            int sourceType,
            string mobile,
            string netType,
            string province,
            string city,
            string referUrl,
            string iPAddress,
            string remoteHost,
            DateTime beginTime,
            DateTime endTime,
            DateTime cancelTime,
            int autoRenew,
            int orderContentType
        )
        {
            _id = id;
            _packageId = packageId;
            _packageTitle = packageTitle;
            _fee = fee;
            _orderTime = orderTime;
            _status = status;
            _payChannel = payChannel;
            _orderCode = orderCode;
            _rebate = rebate;
            _rebateFee = rebateFee;
            _feeId = feeId;
            _balance = balance;
            _bindFee = bindFee;
            _cash = cash;
            _integral = integral;
            _rebateExpression = rebateExpression;
            _userId = userId;
            _userName = userName;
            _clientId = clientId;
            _version = version;
            _userAgent = userAgent;
            _imei = imei;
            _imsi = imsi;
            _channelId = channelId;
            _sourceType = sourceType;
            _mobile = mobile;
            _netType = netType;
            _province = province;
            _city = city;
            _referUrl = referUrl;
            _iPAddress = iPAddress;
            _remoteHost = remoteHost;
            _beginTime = beginTime;
            _endTime = endTime;
            _cancelTime = cancelTime;
            _autoRenew = autoRenew;
            _orderContentType = orderContentType;

        }

        #endregion

        #region Public Properties

        /// <summary>
        /// Id
        /// </summary>
        public int Id
        {
            get { return _id; }
            set { _id = value; }
        }

        /// <summary>
        /// PackageId
        /// </summary>
        public int PackageId
        {
            get { return _packageId; }
            set { _packageId = value; }
        }

        /// <summary>
        /// PackageTitle
        /// </summary>
        public string PackageTitle
        {
            get { return _packageTitle; }
            set { _packageTitle = value; }
        }

        /// <summary>
        /// Fee
        /// </summary>
        public decimal Fee
        {
            get { return _fee; }
            set { _fee = value; }
        }

        /// <summary>
        /// OrderTime
        /// </summary>
        public DateTime OrderTime
        {
            get { return _orderTime; }
            set { _orderTime = value; }
        }

        /// <summary>
        /// Status
        /// </summary>
        public int Status
        {
            get { return _status; }
            set { _status = value; }
        }

        /// <summary>
        /// PayChannel
        /// </summary>
        public int PayChannel
        {
            get { return _payChannel; }
            set { _payChannel = value; }
        }

        /// <summary>
        /// OrderCode
        /// </summary>
        public string OrderCode
        {
            get { return _orderCode; }
            set { _orderCode = value; }
        }

        /// <summary>
        /// Rebate
        /// </summary>
        public decimal Rebate
        {
            get { return _rebate; }
            set { _rebate = value; }
        }

        /// <summary>
        /// RebateFee
        /// </summary>
        public decimal RebateFee
        {
            get { return _rebateFee; }
            set { _rebateFee = value; }
        }

        /// <summary>
        /// FeeId
        /// </summary>
        public int FeeId
        {
            get { return _feeId; }
            set { _feeId = value; }
        }

        /// <summary>
        /// Balance
        /// </summary>
        public decimal Balance
        {
            get { return _balance; }
            set { _balance = value; }
        }

        /// <summary>
        /// BindFee
        /// </summary>
        public decimal BindFee
        {
            get { return _bindFee; }
            set { _bindFee = value; }
        }

        /// <summary>
        /// Cash
        /// </summary>
        public decimal Cash
        {
            get { return _cash; }
            set { _cash = value; }
        }

        /// <summary>
        /// Integral
        /// </summary>
        public int Integral
        {
            get { return _integral; }
            set { _integral = value; }
        }

        /// <summary>
        /// RebateExpression
        /// </summary>
        public string RebateExpression
        {
            get { return _rebateExpression; }
            set { _rebateExpression = value; }
        }

        /// <summary>
        /// ClientLog: UserId.
        /// </summary>
        public int UserId
        {
            get { return _userId; }
            set { _userId = value; }
        }

        /// <summary>
        /// UserLog: UserName.
        /// </summary>
        public string UserName
        {
            get { return _userName; }
            set { _userName = value; }
        }

        /// <summary>
        /// UserLog: ClientId.
        /// </summary>
        public int ClientId
        {
            get { return _clientId; }
            set { _clientId = value; }
        }

        /// <summary>
        /// UserLog: Version.
        /// </summary>
        public string Version
        {
            get { return _version; }
            set { _version = value; }
        }

        /// <summary>
        /// UserLog: User Agent.
        /// </summary>
        public string UserAgent
        {
            get { return _userAgent; }
            set { _userAgent = value; }
        }

        /// <summary>
        /// UserLog: IMEI.
        /// </summary>
        public string IMEI
        {
            get { return _imei; }
            set { _imei = value; }
        }

        /// <summary>
        /// UserLog: IMSI.
        /// </summary>
        public string IMSI
        {
            get { return _imsi; }
            set { _imsi = value; }
        }

        /// <summary>
        /// UserLog: ChannelId.
        /// </summary>
        public string ChannelId
        {
            get { return _channelId; }
            set { _channelId = value; }
        }

        /// <summary>
        /// UserLog: SourceType.
        /// </summary>
        public int SourceType
        {
            get { return _sourceType; }
            set { _sourceType = value; }
        }

        /// <summary>
        /// WebLog: Mobile/IP.
        /// </summary>
        public string Mobile
        {
            get { return _mobile; }
            set { _mobile = value; }
        }

        /// <summary>
        /// WebLog: Mobile/WIFI.
        /// </summary>
        public string NetType
        {
            get { return _netType; }
            set { _netType = value; }
        }

        /// <summary>
        /// WebLog: Province.
        /// </summary>
        public string Province
        {
            get { return _province; }
            set { _province = value; }
        }

        /// <summary>
        /// WebLog: City.
        /// </summary>
        public string City
        {
            get { return _city; }
            set { _city = value; }
        }

        /// <summary>
        /// WebLog: ReferUrl.
        /// </summary>
        public string ReferUrl
        {
            get { return _referUrl; }
            set { _referUrl = value; }
        }

        /// <summary>
        /// WebLog: IP.
        /// </summary>
        public string IPAddress
        {
            get { return _iPAddress; }
            set { _iPAddress = value; }
        }

        /// <summary>
        /// WebLog: Remote Host.
        /// </summary>
        public string RemoteHost
        {
            get { return _remoteHost; }
            set { _remoteHost = value; }
        }

        /// <summary>
        /// BeginTime
        /// </summary>
        public DateTime BeginTime
        {
            get { return _beginTime; }
            set { _beginTime = value; }
        }

        /// <summary>
        /// EndTime
        /// </summary>
        public DateTime EndTime
        {
            get { return _endTime; }
            set { _endTime = value; }
        }

        /// <summary>
        /// CancelTime
        /// </summary>
        public DateTime CancelTime
        {
            get { return _cancelTime; }
            set { _cancelTime = value; }
        }

        /// <summary>
        /// AutoRenew
        /// </summary>
        public int AutoRenew
        {
            get { return _autoRenew; }
            set { _autoRenew = value; }
        }

        /// <summary>
        /// OrderContentType
        /// </summary>
        public int OrderContentType
        {
            get { return _orderContentType; }
            set { _orderContentType = value; }
        }

        #endregion
    }
}

