...
namespace Utility
{
    public static class StringHelper
    {
        ...
        private const string ChannelRouteUrlPattern = "(?<begin>\\/)(?<channelName>{0})(?<middle>\\/)(?<channelId>(?!^[a-zA-Z]+$)[0-9a-zA-Z]+)(?<end>(\\/)?(\\?)?)";
        ...
        public static string GetChannelRouteUrl(this string url, string paramText, string paramValue = "")
        {
            ...
            Regex reg = new Regex(string.Format(ChannelRouteUrlPattern, paramText), RegexOptions.IgnoreCase);
        ...
        private const string SpliceUrlPattern = "(?<begin>[?|&])(?<param>{0}=[^&]*)(?<end>[&]?)";
        ...
        public static string SpliceUrl(this string url, string paramText, string paramValue)
        {
            ...
            Regex reg = new Regex(string.Format(SpliceUrlPattern, paramText), RegexOptions.IgnoreCase);
        ...
        /// <param name="newValue">"\r\n　　"</param>
        /// <returns></returns>
        public static string ConvertTxt(string content, string oldValue, string newValue)
        {
            if (string.IsNullOrEmpty(content)) return "";
        ...
        private const string SignText = "<span id=\"{0}\" style=\"display:none;\">{1}</span>";
        ...
        /// <returns></returns>
        public static string ConvertAndSignTxt(string content, string id, string sign)
        {
            if (string.IsNullOrEmpty(content)) return "";

            content = content.Replace("　", "").Replace(" ", "").Replace("\r\n", "☀").Trim();

            if (!string.IsNullOrEmpty(sign) && !string.IsNullOrEmpty(content))
            {
                string contentSign = string.Format(SignText, id, sign);
                int contentLength = content.Length;
                int contentIndex = 0;
                if (RandomHelper.Get(contentLength > 3 ? contentLength / 2 : 1, contentLength - 1, out contentIndex))
                {
                    content = content.Insert(contentIndex, contentSign);
                }
            }
        ...
        public static IList<T> Reverse<T>(IList<T> list)
        {
            ...
            {
                IEnumerable<T> l = list.Reverse();
                if (!l.IsNullOrEmpty<T>())
                {
                    return l.ToList();
                }
            }
