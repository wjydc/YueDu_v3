...
namespace YueDu.Controllers
{
    public class ChapterController : ChapterDetailController
    {
        ...
        private string GetContent(int contentType, string chapterName, string filePath, string fileName)
        {
            //正文中插入用户信息
            string encryptInfo = string.IsNullOrEmpty(currentUser.UserName) ? "" : string.Concat("14", ";", currentUser.UserName.Substring(1));

            return StringHelper.ConvertAndSignTxt((contentType == (int)Constants.Novel.ContentType.小说) ? FileHelper.ReadFile(FileHelper.MergePath("\\", new string[] { filePath, fileName }), chapterName) : "", Constants.SecurityKey.userAESId.ToLower(), encryptInfo);
        }
